...
namespace gulshatLABA
{
    public partial class WorkForm : Form
    {
        ...
        void LoadShopsForTovars()
        {
            ...
            {
                DataTable table = myDB.callProc("ShopsForTovar", new List<(string, object)> { ("@TovarID", tovars[i]) });
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                if (dataGridView1.Rows[i].Cells[9].Value != null)
                {
                    ...
                    myDB.Update(
                        "TovarInZakaz", new List<(string, object)> { ("ID_shop", shopID) },
                        new List<(string, object)> { ("ID_Tovar", tovarID), ("ID_Zakaz", zakazID) }
